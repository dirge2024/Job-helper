...
function ReviewsPage() {
  ...
  const deleteReview = async (item: ReviewItem) => {
    const updatedRecord = { ...item.record, interviewReviews: (item.record.interviewReviews ?? []).filter(review => review.id !== item.review.id), updatedAt: new Date().toISOString() };
    const response = await MessageService.sendMessage({ type: 'UPDATE_APPLICATION_RECORD', payload: updatedRecord });
    if (!response.success) { setErrorText(response.error || '删除面经失败'); return; }
    setRecords(current => current.map(record => record.id === updatedRecord.id ? updatedRecord : record));
    if (selected?.reviewId === item.review.id) setSelected(null);
    setNotice('面经已删除');
  ...
  return <>
    ...
    <div className="reviews-layout">
      ...
      <aside className="review-list-card"><div className="review-list-tools"><input value={keyword} onChange={event => setKeyword(event.target.value)} placeholder="搜索面经..." /><select aria-label="筛选面经状态" value={filter} onChange={event => setFilter(event.target.value as 'all' | 'pending' | 'completed')}><option value="all">全部</option><option value="pending">待复盘</option><option value="completed">已记录</option></select><button type="button" className="dashboard-primary-action" onClick={() => setAdding(true)}>+ 添加面经</button></div><div className="review-list">{reviewItems.length === 0 ? <p>暂无面经记录</p> : reviewItems.map(item => <div key={item.review.id} className={selected?.reviewId === item.review.id ? 'review-list-item is-active' : 'review-list-item'}><button type="button" className="review-list-select" onClick={() => setSelected({ recordId: item.record.id, reviewId: item.review.id })}><b>{item.record.companyName || '未填写公司'} · {item.review.stage}</b><span>{item.record.jobTitle || '未填写岗位'}</span><small>{item.review.status === 'pending' ? '待复盘' : '已记录'} · {item.review.updatedAt.slice(0, 10)}</small></button><button type="button" className="review-list-delete" aria-label={`删除${item.record.companyName || '该条'}${item.review.stage}面经`} onClick={() => setDeletingReview(item)}>删除</button></div>)}</div></aside>
    </div>
    {adding && <AddReviewModal records={records} onClose={() => setAdding(false)} onSave={addReview} />}
    {deletingReview && <div className="dashboard-modal-backdrop" role="presentation" onMouseDown={() => setDeletingReview(null)}><section className="dashboard-modal confirm-modal" role="dialog" aria-modal="true" aria-labelledby="delete-review-title" onMouseDown={event => event.stopPropagation()}><p className="dashboard-card-eyebrow">DELETE INTERVIEW NOTE</p><h2 id="delete-review-title">确认删除面经？</h2><p>{deletingReview.record.companyName || '该公司'} · {deletingReview.review.stage}</p><footer><button type="button" className="dashboard-secondary-action" onClick={() => setDeletingReview(null)}>取消</button><button type="button" className="schedule-cancel-action" onClick={() => void deleteReview(deletingReview)}>确定删除</button></footer></section></div>}
  </>;
}
